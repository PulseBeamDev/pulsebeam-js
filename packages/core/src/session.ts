...
/**
 * Represents a remote participant's video.
 * The UI binds to this, setting 'height' via ResizeObservers.
 */
...
export class Session {
  ...
  constructor(config: SessionConfig) {
    ...
    this.devices = new DeviceManager(this.adapter, (kind, track) => {
      this.handleLocalTrackChange(kind, track);
    });
  ...
  close() {
    ...
    this.pc.close();
    this.devices.muteCamera();
    this.devices.muteMicrophone();
  }

  getSlot(trackId: string): VirtualSlot | undefined {
    return this.virtualSlots.get(trackId);
  }
  ...
  private handleLocalTrackChange(kind: "video" | "audio", track: MediaStreamTrack | null) {
    const trans = kind === "video" ? this.videoTrans : this.audioTrans;
    trans.sender.replaceTrack(track).catch(err => {
      console.error(`[Session] Failed to replace ${kind} track`, err);
    });
  }
  ...
  private async connectInternal(endpoint: string, room: string) {
    try {
      ...
      if (!offer.sdp) throw new Error("SFU Offer generation failed");
